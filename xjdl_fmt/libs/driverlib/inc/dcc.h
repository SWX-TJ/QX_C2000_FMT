...
#define DCC_REG_WORD_MASK (0xFFFFFU)
...
{
    EALLOW;
    HWREGH(base + DCC_O_CLKSRC1) = (HWREGH(base + DCC_O_CLKSRC1) & (DCC_REG_BYTE_MASK << 4U))
                                   | ((DCC_ENABLE_VALUE << 12U) | (uint16_t)source);
    EDIS;
}
...
{
    EALLOW;
    HWREGH(base + DCC_O_CLKSRC0)
        = (HWREGH(base + DCC_O_CLKSRC0) & ~(DCC_CLKSRC0_CLKSRC0_M)) | (uint16_t)source;
    EDIS;
}
...
static inline void DCC_setCounterSeeds(
    uint32_t base, uint32_t counter0, uint32_t validCounter0, uint32_t counter1)
{
    ...
    EALLOW;
    // Set Counter 0 Seed
    HWREG(base + DCC_O_CNTSEED0) = counter0 & DCC_REG_WORD_MASK;
    // HWREGH(base + DCC_O_CNTSEED0 + 4U) = (HWREGH(base + DCC_O_CNTSEED0 + 4U) & DCC_SEED_REG_MASK)
    ...
    // Set Counter 1 Seed
    HWREG(base + DCC_O_CNTSEED1)      = counter1 & DCC_REG_WORD_MASK;
//    HWREGH(base + DCC_O_CNTSEED1 + 4U) = (HWREGH(base + DCC_O_CNTSEED1 + 4U) & DCC_SEED_REG_MASK)
...
//
//*****************************************************************************
extern float32_t DCC_measureClockFrequency(uint32_t base, DCC_Count1ClockSource clock1,
    DCC_Count0ClockSource clock0, float32_t freq0, float32_t tolerance, float32_t freqerr,
    float32_t freq_sysclk);
