...
typedef enum
{
    SPI_PROT_POL0PHA0 = 0x0U,
    SPI_PROT_POL0PHA1 = 0x1U,
    SPI_PROT_POL1PHA0 = 0x2U,
    SPI_PROT_POL1PHA1 = 0x3U
...
typedef enum
{
    ...
    SPI_FIFO_RX1       = 0x0001U, //!< Receive FIFO 1/8 full
    SPI_FIFO_RX2       = 0x0002U, //!< Receive FIFO 2/8 full
    SPI_FIFO_RX3       = 0x0003U, //!< Receive FIFO 3/8 full
    SPI_FIFO_RX4       = 0x0004U, //!< Receive FIFO 4/8 full
    SPI_FIFO_RX5       = 0x0005U, //!< Receive FIFO 5/8 full
    SPI_FIFO_RX6       = 0x0006U, //!< Receive FIFO 6/8 full
    SPI_FIFO_RX7       = 0x0007U, //!< Receive FIFO 7/8 full
    SPI_FIFO_RX8       = 0x0008U, //!< Receive FIFO 8/8 full
...
{
    return ((base == SPIA_MASTER_BASE) || (base == SPIA_SLAVE_BASE) || (base == SPIB_MASTER_BASE)
            || (base == SPIB_SLAVE_BASE) || (base == SPIC_MASTER_BASE) || (base == SPIC_SLAVE_BASE)
            || (base == SPID_MASTER_BASE) || (base == SPID_SLAVE_BASE));
}
#endif
...
static inline void SPI_setcharLength(uint32_t base, uint16_t charLength)
{
    ...
    ((volatile struct SPI_REGS *)base)->CTRL0.bit.DFS_32 = charLength;
...
static inline void SPI_setFIFOInterruptLevel(
    uint32_t base, SPI_TxFIFOLevel txLevel, SPI_RxFIFOLevel rxLevel)
{
    //
    // Check the arguments.
    //
...
static inline void SPI_enableDMA(uint32_t base, bool tx, bool rx)
{
    //
    // Check the arguments.
    //
    ...
    SPI_disableModule(base);
    if (tx)
        ((volatile struct SPI_REGS *)base)->DMAC.bit.TDMAE = 1;

    if (rx)
        ((volatile struct SPI_REGS *)base)->DMAC.bit.RDMAE = 1;
    SPI_enableModule(base);
}

static inline void SPI_disableDMA(uint32_t base, bool tx, bool rx)
{
    //
    // Check the arguments.
    //
    ...
    SPI_disableModule(base);
    if (tx)
        ((volatile struct SPI_REGS *)base)->DMAC.bit.TDMAE = 0;

    if (rx)
        ((volatile struct SPI_REGS *)base)->DMAC.bit.RDMAE = 0;
    SPI_enableModule(base);
}
...
//
//*****************************************************************************
static inline void SPI_writeDataNonBlocking(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline uint32_t SPI_readDataNonBlocking(uint32_t base)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline void SPI_writeDataBlockingFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline uint32_t SPI_readDataBlockingFIFO(uint32_t base)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline void SPI_writeDataBlockingNonFIFO(uint32_t base, uint32_t data)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
static inline uint32_t SPI_readDataBlockingNonFIFO(uint32_t base)
{
    //
    // Check the arguments.
    //
...
//
//*****************************************************************************
extern void SPI_setConfig(uint32_t base, uint32_t lspclkHz, SPI_TransferProtocol protocol,
...
//
//*****************************************************************************
extern uint16_t SPI_receive16Bits(
...
//
//*****************************************************************************
extern uint32_t SPI_receive24Bits(
    uint32_t base, SPI_endianess endianness, uint16_t dummyData, uint16_t txDelay);
...
//
//*****************************************************************************
extern uint32_t SPI_receive32Bits(
    uint32_t base, SPI_endianess endianness, uint16_t dummyData, uint16_t txDelay);
...
//
//*****************************************************************************
extern void SPI_pollingFIFOTransaction(uint32_t base, uint16_t charLength, uint16_t *pTxBuffer,
