...
{
    return ((base == ECAP1_BASE) || (base == ECAP2_BASE) || (base == ECAP3_BASE)
            || (base == ECAP4_BASE) || (base == ECAP5_BASE) || (base == ECAP6_BASE)
            || (base == ECAP7_BASE));
}
#endif
...
//
//*****************************************************************************
static inline void ECAP_setEventPolarity(
    uint32_t base, ECAP_Events event, ECAP_EventPolarity polarity)
{
    ...
    HWREGH(base + ECAP_O_ECCTL1)
        = (HWREGH(base + ECAP_O_ECCTL1) & ~(1U << shift)) | ((uint16_t)polarity << shift);
    EDIS;
}
...
static inline void ECAP_setCaptureMode(uint32_t base, ECAP_CaptureMode mode, ECAP_Events event)
{
    ...
    HWREGH(base + ECAP_O_ECCTL2)
        = ((HWREGH(base + ECAP_O_ECCTL2) & (~ECAP_ECCTL2_CONT_ONESHT)) | (uint16_t)mode);
...
static inline void ECAP_setSyncOutMode(uint32_t base, ECAP_SyncOutMode mode)
{
    ...
    //
    HWREGH(base + ECAP_O_ECCTL2)
        = ((HWREGH(base + ECAP_O_ECCTL2) & (~ECAP_ECCTL2_SYNCO_SEL_M)) | (uint16_t)mode);
    EDIS;
}
...
static inline void ECAP_setAPWMPolarity(uint32_t base, ECAP_APWMPolarity polarity)
{
    ...
    EALLOW;
    HWREGH(base + ECAP_O_ECCTL2)
        = ((HWREGH(base + ECAP_O_ECCTL2) & ~ECAP_ECCTL2_APWMPOL) | (uint16_t)polarity);
    EDIS;
}
...
static inline void ECAP_selectECAPInput(uint32_t base, ECAP_InputCaptureSignals input)
{
    ...
    HWREGH(base + ECAP_O_ECCTL0)
        = ((HWREGH(base + ECAP_O_ECCTL0) & ~ECAP_ECCTL0_INPUTSEL_M) | (uint16_t)input);
    EDIS;
}
...
static inline ECAP_Events ECAP_getModuloCounterStatus(uint32_t base)
{
    ...
    counterStatusValue
        = (((HWREGH(base + ECAP_O_ECCTL2) & ECAP_ECCTL2_MODCNTRSTS_M) >> ECAP_ECCTL2_MODCNTRSTS_S));
