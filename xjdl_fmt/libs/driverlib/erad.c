...
void ERAD_configCounterInCountingMode(uint32_t base, ERAD_Counter_Config config_params)
{
    ...
    HWREGH(base + ERAD_O_CTM_CNTL)
        = (HWREGH(base + ERAD_O_CTM_CNTL)
              & (~(ERAD_CTM_CNTL_RTOSINT | ERAD_CTM_CNTL_STOP | ERAD_CTM_CNTL_RST_ON_MATCH
                   | ERAD_CTM_CNTL_EVENT_MODE | ERAD_CTM_CNTL_START_STOP_MODE)))
...
//
//*****************************************************************************
void ERAD_configCounterInStartStopMode(uint32_t base, ERAD_Counter_Config config_params,
...
void ERAD_profile(ERAD_Profile_Params config_params)
{
    ...
    ERAD_Counter_Input_Event start
        = (ERAD_Counter_Input_Event)(ERAD_BUSCOMP_BASE_TO_EVENT(config_params.busComp_base1));
    ERAD_Counter_Input_Event stop
        = (ERAD_Counter_Input_Event)(ERAD_BUSCOMP_BASE_TO_EVENT(config_params.busComp_base2));
    ERAD_configCounterInStartStopMode(config_params.counter_base, counter_params, start, stop);
    ...
    ERAD_AddressHit_Params config_params, uint32_t busComp_base, uint32_t counter_base)
{
    ...
    counter_params.rst_on_match = 0;
    counter_params.reference    = 0x0;
    counter_params.event = (ERAD_Counter_Input_Event)(ERAD_BUSCOMP_BASE_TO_EVENT(busComp_base));
    ...
    //
    ERAD_enableModules(
        ERAD_getBusCompInstance(busComp_base) | ERAD_getCounterInstance(counter_base));
}
