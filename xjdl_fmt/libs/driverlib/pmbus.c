...
uint16_t PMBus_pecCalculate(uint16_t *buffer, uint16_t nBytes)
{
	register uint16_t i;
	register uint16_t parity = 0U;
	register uint16_t tableIndex;
	register uint16_t accumulator;
	uint16_t slave_addr = (PmbusaRegs.IC_SAR.bit.IC_SAR & 0xFF) << 1;
	uint16_t slave_rw = (PmbusaRegs.IC_RAW_INTR_STAT.bit.R_RD_REQ) ? 1:0;

	//
	// PEC is always calculated first with the address + R/W bit
	//
	tableIndex  = slave_addr + slave_rw;

	accumulator = PMBus_crc8Table[tableIndex];
...
//          );
    ...
    intStatus = PmbusaRegs.IC_SMBUS_INTR_STAT.all & 0x000003FFU;
...
void PMBus_initTargetMode(uint32_t base, uint16_t address, uint16_t mask)
{
    ...
    PmbusaRegs.IC_CON.bit.IC_SLAVE_DISABLE = 0;
    PmbusaRegs.IC_CON.bit.IC_RESTART_EN = 1;
    PmbusaRegs.IC_CON.bit.IC_10BITADDR_MASTER = 0;
    PmbusaRegs.IC_CON.bit.IC_10BITADDR_SLAVE = 0;
    PmbusaRegs.IC_CON.bit.MASTER_MODE = 0;

    PmbusaRegs.IC_SAR.bit.IC_SAR = (address&0xFE);

    PmbusaRegs.IC_ENABLE.bit.ENABLE = 1;
...
void PMBus_configTarget(uint32_t base, uint32_t configWord)
{
    ...
    if(PMBUS_PMBTCR_PEC_ENA & configWord)
    {
    	PEC_ENA_FLAG = 1;
    }
    else
    ...
    // PMBUS_PMBTCR_MAN_TARGET_ACK+PMBUS_PMBTCR_MAN_CMD+PMBUS_PMBTCR_RX_BYTE_ACK_CNT_M are combined with
    // manual ack,can't be resolved
                   ...
                   bool txPEC)
{
    ...
    if(txPEC == 1U)
    {
    	pecValue = PMBus_pecCalculate(buffer,nBytes);
    	while(i < nBytes)
		{
			PmbusaRegs.IC_DATA_CMD.bit.DATA = *pBuffer;
			i++;
			pBuffer++;
		}
    	PmbusaRegs.IC_DATA_CMD.bit.STOP = 1;
    	PmbusaRegs.IC_DATA_CMD.bit.DATA = pecValue;
    }
    else
    {
    	while(i < nBytes-1)
...
void PMBus_initControllerMode(uint32_t base)
{
    ...
    PmbusaRegs.IC_ENABLE.bit.ENABLE = 0;
	PmbusaRegs.IC_CON.bit.IC_SLAVE_DISABLE = 1;
	PmbusaRegs.IC_CON.bit.IC_RESTART_EN = 1;
	PmbusaRegs.IC_CON.bit.IC_10BITADDR_MASTER = 0;
	PmbusaRegs.IC_CON.bit.IC_10BITADDR_SLAVE = 0;
	PmbusaRegs.IC_CON.bit.MASTER_MODE = 1;
                ...
                uint16_t byteCount, uint16_t pec)
{
    ...
    // PEC is always calculated first with the address + R/W bit
    //
    tableIndex  = PmbusaRegs.IC_TAR.bit.TAR;
...
PMBus_getData(uint32_t base, uint16_t *buffer, uint32_t status)
{
    ...
    //
    num_rx_bytes = PmbusaRegs.IC_RXFLR;

    if(num_rx_bytes != 0U)
    {
        temp = PmbusaRegs.IC_DATA_CMD.bit.DATA;
        for(i = 0U; i < num_rx_bytes; i++)
        {
            buffer[i] = (uint16_t)(temp & 0x00FF);
        }
    }
...
PMBus_putControllerData(uint32_t base, uint16_t *buffer, uint16_t nBytes)
{
    ...
    while(i < nBytes)
    {
    	PmbusaRegs.IC_DATA_CMD.bit.DATA = ((*pBuffer) & 0xFF);
        i++;
    }
}
                  ...
                  uint32_t moduleFrequency)
{
    ...
    uint32_t busIdle, setupTime,clockLowLimit;
    bool status = false;
    ...
    switch(mode)
    {
        ...
        case PMBUS_CLOCKMODE_STANDARD:
        	PmbusaRegs.IC_CON.bit.SPEED = 1;
        	PmbusaRegs.IC_UFM_SPKLEN.bit.UFM_SPKLEN = 1;
            clockHighLimit    = (4000U / timePeriod) - 8U;
        	clockLowLimit     = (4700U / timePeriod) - 1U;
        break;
    ...
    if(status)
    {
    	PmbusaRegs.IC_SS_SCL_HCNT.bit.SS_SCL_HCNT = clockHighLimit;
    	PmbusaRegs.IC_SS_SCL_LCNT.bit.SS_SCL_LCNT = clockLowLimit;
    }
