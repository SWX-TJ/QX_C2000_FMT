...
void EPWM_configureSignal(uint32_t base, const EPWM_SignalParams *signalParams)
{
    ...
    else if (signalParams->tbCtrMode == EPWM_COUNTER_MODE_DOWN)
    {
        tbPrdVal = (uint16_t)((tbClkInHz / signalParams->freqInHz) - 1.0f);
        cmpAVal
            = (uint16_t)((tbPrdVal + 1U) - ((float32_t)signalParams->dutyValA * (tbPrdVal + 1U)));
        cmpBVal
            = (uint16_t)((tbPrdVal + 1U) - ((float32_t)signalParams->dutyValB * (tbPrdVal + 1U)));
    }
    else if ((signalParams->tbCtrMode == EPWM_COUNTER_MODE_UP_DOWN))
    {
        tbPrdVal = (uint16_t)(tbClkInHz / (2.0f * signalParams->freqInHz));
        cmpAVal
            = (uint16_t)(((float32_t)tbPrdVal - ((float32_t)(signalParams->dutyValA * tbPrdVal)))
                         + 0.5f);
        cmpBVal
            = (uint16_t)(((float32_t)tbPrdVal - ((float32_t)(signalParams->dutyValB * tbPrdVal)))
                         + 0.5f);
    }
    ...
    if (signalParams->tbCtrMode == EPWM_COUNTER_MODE_UP)
    {
        ...
        EPWM_setActionQualifierAction(
            base, EPWM_AQ_OUTPUT_A, EPWM_AQ_OUTPUT_HIGH, EPWM_AQ_OUTPUT_ON_TIMEBASE_ZERO);

        //
        // Clear PWMxA on event A, up count
        //
        EPWM_setActionQualifierAction(
            base, EPWM_AQ_OUTPUT_A, EPWM_AQ_OUTPUT_LOW, EPWM_AQ_OUTPUT_ON_TIMEBASE_UP_CMPA);
        ...
        else
        {
            ...
            //
            EPWM_setActionQualifierAction(
                base, EPWM_AQ_OUTPUT_B, EPWM_AQ_OUTPUT_HIGH, EPWM_AQ_OUTPUT_ON_TIMEBASE_ZERO);
            //
    ...
    else if (signalParams->tbCtrMode == EPWM_COUNTER_MODE_UP_DOWN)
    {
        ...
        //
        EPWM_setActionQualifierAction(
            base, EPWM_AQ_OUTPUT_A, EPWM_AQ_OUTPUT_LOW, EPWM_AQ_OUTPUT_ON_TIMEBASE_ZERO);
